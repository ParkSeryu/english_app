...
| Ebbinghaus/SRS interval policy | T-003 | `complete/` | `complete/spaced-repetition-interval-policy/prd.md` | Focused scheduling, memory-store, and memorize-card tests | PR #5 + follow-ups | Completed 4-button SRS policy: `모름` keeps today-due, `어려움` steps down, `알긴암` keeps the interval, `쉬움` steps up to 365 days. |

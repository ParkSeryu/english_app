...
| Progress reset | Replacing target standard sets deletes their Day progress and targeted Prenovice/Novice Pop progress, while preserving source and Premium rows. |
...
| Migration rollback | Inventory, payload, or postcondition failures roll back the full production replacement and leave no partial progress reset. |
...
| Production readback | Main/production readback confirms 44 v2 payloads, semantic JSON/hash equality, source hashes, exact Korean text, and cleared targeted progress. |

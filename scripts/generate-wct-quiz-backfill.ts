...
const DEV_PROJECT_REF = "uixpyibcpleuwsgemdno";
...
async function readDevBooks(client: SupabaseClient): Promise<BackfillBook[]> {
...
function requiredEnv(
  ...
  if (!value) throw new Error(`${DEV_ENV_FILE} is missing ${key}`);
  return value;
}

function createDevClient() {
  const env = loadEnvFile(path.join(ROOT, DEV_ENV_FILE));
  ...
  if (projectRef !== DEV_PROJECT_REF) {
    throw new Error(
      `${DEV_ENV_FILE} points to ${projectRef}, expected ${DEV_PROJECT_REF}`
    );
  }
...
async function main(args: readonly string[]) {
  ...
  const envName = argValue(args, "--env") ?? "dev";
  if (envName !== "dev") {
    throw new Error("WCT quiz backfill supports only --env dev");
  }
  if (command !== "generate" && command !== "verify") {
    throw new Error(
      "Usage: generate-wct-quiz-backfill.ts <generate|verify> --env dev [--output file]"
    );
  }

  const { client, projectRef } = createDevClient();
  const books = await readDevBooks(client);
